...
module {
  ...
  // CHECK: [[ARG0:%.+]] = iree.placeholder for "interface buffer" {binding = @legacy_io::@arg0} : memref<2x2xf32>
  // CHECK: [[ARG1:%.+]] = iree.placeholder for "interface buffer" {binding = @legacy_io::@arg1} : memref<2x2xf32>
  // CHECK: [[ARG2:%.+]] = iree.placeholder for "interface buffer" {binding = @legacy_io::@ret0} : memref<2x2xf32>
  func @element_wise() {
    %c0 = constant 0 : index
