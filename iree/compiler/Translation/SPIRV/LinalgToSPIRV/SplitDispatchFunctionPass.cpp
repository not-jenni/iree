...
/// Returns true if the Linalg ops can be distribute to multiple kernels.
bool canDistributeLinalgOps(MutableArrayRef<linalg::LinalgOp> linalgOps) {
  // Require all buffers operated by Linalg ops directly come from interface
  // placeholder.
  for (linalg::LinalgOp linalgOp : linalgOps) {
    if (!linalgOp.hasBufferSemantics()) return false;
    for (Value buffer : linalgOp.getInputsAndOutputBuffers()) {
      Operation *def = buffer.getDefiningOp();
      // Penetrate potential shape annotation ops.
      if (auto tieShapeOp = dyn_cast_or_null<Shape::TieShapeOp>(def)) {
        def = tieShapeOp.operand().getDefiningOp();
      }
      if (!isa_and_nonnull<IREE::PlaceholderOp>(def)) return false;
    }
  }
...
/// Collects all the operations in the closure of the given `rootOp` into
/// `closure`.
void collectClosure(Operation *rootOp,
  ...
  while (!workList.empty()) {
    ...
    if (!closure.insert(curOp).second) continue;
...
struct SplitKernelPattern : public OpRewritePattern<FuncOp> {
  using OpRewritePattern<FuncOp>::OpRewritePattern;
  LogicalResult matchAndRewrite(FuncOp oldFn,
...
void SplitDispatchFunctionPass::runOnOperation() {
  ...
  patterns.insert<SplitKernelPattern>(&context);
  applyOpPatternsAndFold(functions.front(), patterns);
}

LogicalResult SplitKernelPattern::matchAndRewrite(
    FuncOp oldFn, PatternRewriter &rewriter) const {
  ...
  if (!llvm::hasSingleElement(oldFn.getBlocks())) {
    return rewriter.notifyMatchFailure(oldFn, "expected only one block");
  }

  // The dispatch function should have more than one Linalg structured ops.
  // Otherwise there is nothing to do.
  if (hasZeroOrOneLinalgOp(oldFn.getBlocks().front())) return failure();

  // Collect all Linalg ops for distributing.
  SmallVector<linalg::LinalgOp, 4> linalgOps =
      llvm::to_vector<4>(oldFn.getBlocks().front().getOps<linalg::LinalgOp>());
  if (!canDistributeLinalgOps(linalgOps)) {
    return rewriter.notifyMatchFailure(
        oldFn, "cannot distribute Linalg ops into multiple kernels");
  }
  ...
  for (const auto &linalgOp : llvm::enumerate(linalgOps)) {
    // Createa a new function for hosting this Linalg op.
    ...
    auto newFn = rewriter.create<FuncOp>(loc, newFnName, oldFn.getType(),
  ...
  for (const std::string &kernel : splitKernels) {
    entryPoints.emplace_back(rewriter.getStringAttr(kernel));
  }
  moduleOp.setAttr(getEntryPointScheduleAttrName(),
                   rewriter.getArrayAttr(entryPoints));

  rewriter.eraseOp(oldFn);
  return success();
}
