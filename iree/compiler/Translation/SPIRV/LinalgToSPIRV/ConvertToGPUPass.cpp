...
class GPUGlobalInvocationId;
class GPUGlobalNprocs;
}  // namespace

template <typename GPUIdOp, typename GPUNProcsOp>
static IdAndNprocs getGPUIdAndNprocs(Location loc, StringRef dim,
                                     ConversionPatternRewriter &rewriter) {
  Type indexType = rewriter.getIndexType();
  return {rewriter.create<GPUIdOp>(loc, indexType, rewriter.getStringAttr(dim)),
          rewriter.create<GPUNProcsOp>(loc, indexType,
                                       rewriter.getStringAttr(dim))};
}

template <>
IdAndNprocs getGPUIdAndNprocs<GPUGlobalInvocationId, GPUGlobalNprocs>(
...
/// dimension.
template <typename GPUIdOp, typename GPUDimOp>
static LogicalResult mapToProcessor(ConversionPatternRewriter &rewriter,
  ...
  for (unsigned i = 0; i < numLoops; ++i) {
    IdAndNprocs idAndNprocs =
        getGPUIdAndNprocs<GPUIdOp, GPUDimOp>(loc, dims[i], rewriter);
    pid.insert(pid.begin(), idAndNprocs.id);
    nprocs.insert(nprocs.begin(), idAndNprocs.nprocs);
  }
...
/// pid = blockIdx * blockDim + gridIdx. nprocs = blockDim * gridDim.
static LogicalResult mapToGlobalInvocationId(
    ConversionPatternRewriter &rewriter, loop::ParallelOp pLoopOp) {
  return mapToProcessor<GPUGlobalInvocationId, GPUGlobalNprocs>(rewriter,
                                                                pLoopOp);
}
